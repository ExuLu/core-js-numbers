...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const squareXDifference = (x1 - x2) * (x1 - x2);
  const squareYDifference = (y1 - y2) * (y1 - y2);
  const sumOfDifferences = squareXDifference + squareYDifference;
  const distance = Math.sqrt(sumOfDifferences);
  return distance;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angleCos = scalarMultiplication / (vectorModule1 * vectorModule2);
...
function getParallelepipedDiagonal(a, b, c) {
  const squareDiagonal = a * a + b * b + c * c;
  const diagonal = Math.sqrt(squareDiagonal);
  return diagonal;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const result = Math.round(num / rounder) * rounder;
  return result;
}
...
function getFibonacciNumber(index) {
  let prePreviousNumber = 0;
  let previousNumber = 1;
  const fibonacci = Array.from({ length: index + 1 }).map((item, i) => {
    let curr;
    if (i < 2) {
      curr = i === 0 ? prePreviousNumber : previousNumber;
    }
    if (i >= 2) {
      curr = prePreviousNumber + previousNumber;
...
function getSumOfDigits(num) {
  ...
  const sum = arrayOfDigits.reduce((acc, curr) => acc + +curr, 0);
  return sum;
}
...
function isPowerOfTwo(num) {
  const squareRoot = Math.log2(num);
